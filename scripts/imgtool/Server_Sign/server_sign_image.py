        ...
        if username.strip() =='':
            ...
            if server_home == None or worker == None :
                ...
                if 'bin' in arr :
                    ...
                    if user_pwd is None:
                        str_arg = "signclient.cmd signdocument -workername " + str(worker) + " -servlet " + "/signserver/worker/" + str(worker) + " -truststore "+ str(server_home) + "/eaton-truststore.jks -truststorepwd eaton -infile " + str(file_path) + " -hosts signserverp1.tcc.etn.com,signserverp2.tcc.etn.com -port 8443 -username " + str(username) + " -outfile " + str(output_file)
                    else :
                        str_arg = "signclient.cmd signdocument -workername " + str(worker) + " -servlet " + "/signserver/worker/" + str(worker) + " -truststore "+ str(server_home) + "/eaton-truststore.jks -truststorepwd eaton " + " -password " + str(user_pwd) + " -infile " + str(file_path) + " -hosts signserverp1.tcc.etn.com,signserverp2.tcc.etn.com -port 8443 -username " + str(username) + " -outfile " + str(output_file)
